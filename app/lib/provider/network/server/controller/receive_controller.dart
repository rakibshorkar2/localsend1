...
class ReceiveController {
      ...
      (savedToGallery, filePath) = await saveFile(
        ...
        onProgress: (savedBytes) {
          if (receivingFile.file.size != 0) {
            ...
            progressUpdateCount++;
            if (progressUpdateCount % 3 == 0) {
              _updateLiveActivityForReceive();
            }
          }
        },
  ...
  void _startLiveActivityForReceive() {
    final session = server.getStateOrNull()?.session;
  ...
  void _updateLiveActivityForReceive() {
    final session = server.getStateOrNull()?.session;
    ...
    for (final f in activeFiles) {
      ...
      totalBytes += size;
      currBytes += (server.ref.notifier(progressProvider).getProgress(sessionId: session.sessionId, fileId: f.file.id) * size).round();
    }

    final remainingBytes = totalBytes - currBytes;
    final speedInBytes = getFileSpeed(start: session.startTime!, end: now, bytes: currBytes);
