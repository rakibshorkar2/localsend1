...
import 'package:localsend_app/util/native/ios_live_activity_service.dart';
...
class SendNotifier extends Notifier<Map<String, SendSessionState>> {
  ...
  Future<void> startSession({
    ...
    _startLiveActivityForSend(sessionId);

    await _sendLoop(ref, sessionId, target, sendingFiles);
  }

  void _startLiveActivityForSend(String sessionId) {
    final session = state[sessionId];
    if (session == null) return;
    final firstFile = session.files.values.firstWhereOrNull((f) => f.token != null)?.file;
    if (firstFile == null) return;
    final totalBytes = session.files.values
        .where((f) => f.token != null)
        .fold<int>(0, (sum, f) => sum + f.file.size);
    // ignore: discarded_futures
    IosLiveActivityService.start(
  ...
  void _updateLiveActivityForSession(String sessionId) {
    final session = state[sessionId];
    if (session == null || session.startTime == null) return;
    _updateLiveActivity(session: session, provider: ref.notifier(progressProvider));
  }

  static void _updateLiveActivity({
    required SendSessionState session,
    required ProgressNotifier provider,
  }) {
    final now = DateTime.now().millisecondsSinceEpoch;
    final activeFiles = session.files.values.where((f) => f.token != null).toList();
    ...
    for (final f in activeFiles) {
      ...
      totalBytes += size;
      currBytes += (provider.getProgress(sessionId: session.sessionId, fileId: f.file.id) * size).round();
    }

    final remainingBytes = totalBytes - currBytes;
    final speedInBytes = session.startTime != null
        ? getFileSpeed(start: session.startTime!, end: now, bytes: currBytes)
        : 0;
    final remainingSeconds = speedInBytes > 0 ? remainingBytes / speedInBytes : 0.0;

    // ignore: discarded_futures
    IosLiveActivityService.update(
      progress: totalBytes > 0 ? currBytes / totalBytes : 0.0,
      transferredBytes: currBytes,
      totalBytes: totalBytes,
      speed: speedInBytes.toDouble(),
  ...
  void _finish({required String sessionId}) {
    ...
    if (state[sessionId]!.status != SessionStatus.sending) {
      ...
      if (hasError) {
        // ignore: discarded_futures
    IosLiveActivityService.end(status: 'failed');
      } else {
        // ignore: discarded_futures
        IosLiveActivityService.end(status: 'completed');
      }
  ...
  Future<bool> sendFile({
    ...
    try {
      ...
      await for (final progress in taskResult.progress) {
        ...
        updateCount++;
        if (updateCount % 3 == 0) {
          _updateLiveActivityForSession(sessionId);
        }
      }
  ...
  void cancelSession(String sessionId) {
    ...
    // ignore: discarded_futures
    IosLiveActivityService.end(status: 'cancelled');
